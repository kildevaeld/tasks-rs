...
// pub trait BoxedClonableSyncTask<I, O, E>:
//     Task<I, Output = O, Error = E, Future = BoxFuture<'static, Result<O, Rejection<I, E>>>> + Send
// {
//     fn box_clone(&self) -> Box<dyn BoxedClonableSyncTask<I, O, E> + Sync>;
// }

// impl<T, R> BoxedClonableSyncTask<R, T::Output, T::Error> for BoxedTask<T>
// where
//     T: Task<R> + Clone + 'static + Send + Sync,
//     T::Future: 'static,
// {
//     fn box_clone(&self) -> Box<dyn BoxedClonableSyncTask<R, T::Output, T::Error> + Sync> {
//         boxtask(self.0.clone())
//     }
// }

// impl<I, O, E> Task<I> for Box<dyn BoxedClonableSyncTask<I, O, E> + Sync> {
//     type Output = O;
//     type Error = E;
//     #[allow(clippy::type_complexity)]
//     type Future = BoxFuture<'static, Result<Self::Output, Rejection<I, Self::Error>>>;
...
// impl<I, O, E> Clone for Box<dyn BoxedClonableSyncTask<I, O, E> + Sync> {
//     fn clone(&self) -> Self {
//         self.as_ref().box_clone()
//     }
// }
